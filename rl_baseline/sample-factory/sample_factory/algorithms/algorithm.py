...
class AlgorithmBase:
    ...
    def add_cli_args(cls, parser):
        ...
        p.add_argument('--reward_dir', default=join(os.getcwd(), 'train_dir/rl_pairs_dataset_random_seed1'), type=str, help='Root dir to load reward model from.')
...
class ReinforcementLearningAlgorithm(AlgorithmBase, ABC):
    ...
    def add_cli_args(cls, parser):
        ...
        # LLM related stuff
        p.add_argument('--extrinsic_reward', default=1.0, type=float, help='To use or not the extrinsic reward')

        p.add_argument('--save_every_sec', default=120, type=int, help='Checkpointing rate')
        p.add_argument('--save_every_steps', default=1e8, type=int, help='Checkpointing rate')
        p.add_argument('--keep_checkpoints', default=3, type=int, help='Number of model checkpoints to keep')
        p.add_argument('--checkpoint_id', default=-1, type=int, help='Checkpoint id to load from folder')
        ...
        p.add_argument('--pop_bl', default=True, type=str2bool, help='To remove or not the bl_stats from the observation')
        # RL
        ...
        p.add_argument('--encoder_custom', default='nle_rgbcrop_encoder', type=str, help='Use custom encoder class from the registry (see model_utils.py)')
