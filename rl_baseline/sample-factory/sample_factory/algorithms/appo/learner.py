...
class LearnerWorker:
    ...
    def init_model(self, timing):
        ...
        if self.cfg.llm_reward > 0.:
            ...
            self.reward_model.load_state_dict(checkpoint_dict['model'])
            self.reward_model.add_mean_var(checkpoint_dict['reward_mean'].item(), checkpoint_dict['reward_var'].item())
            self.reward_model.model_to_device(self.device)
            self.reward_model.share_memory()

        # Load the acting agent
