...
def create_encoder(cfg, obs_space, timing, override_encoder_custom=None):
    encoder_custom = override_encoder_custom if override_encoder_custom else cfg.encoder_custom
    if encoder_custom:
        encoder_cls = ENCODER_REGISTRY[encoder_custom]
...
class PolicyCoreRNN(PolicyCoreBase):
    ...
    def forward(self, head_output, rnn_states, override_is_seq=False):
        is_seq = not torch.is_tensor(head_output) or override_is_seq
        if not is_seq:
...
class PolicyCoreFeedForward(PolicyCoreBase):
    ...
    def forward(self, head_output, fake_rnn_states, override_is_seq=False):
        return head_output, fake_rnn_states


def create_core(cfg, core_input_size, override_use_rnn=None):
    use_rnn = override_use_rnn if override_use_rnn is not None else cfg.use_rnn
    if use_rnn:
