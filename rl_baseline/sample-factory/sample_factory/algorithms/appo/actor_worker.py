...
class ActorState:
    ...
    def record_env_step(self, reward, done, info, rollout_step):
        ...
        if done:
            ...
            report = self.episodic_stats(last_episode_true_reward, last_episode_extra_stats, info)
    ...
    def episodic_stats(self, last_episode_true_reward, last_episode_extra_stats, info):
        ...
        stats['gold_counts'] = info.get('gold_counts', 0)
        stats['stairs_counts'] = info.get('stairs_counts', 0)
        stats['kill_counts'] = info.get('kill_counts', 0)
        stats['dlvl'] = info.get('dlvl', 0)
        stats['gold'] = info.get('gold', 0)
        stats['xlvl'] = info.get('xlvl', 0)
        stats['scout'] = info.get('scout', 0)
...
class ActorWorker:
    ...
    def __init__(
        ...
        task_queue, policy_queues, report_queue, learner_queues, affinity
    ):
        """
        Ctor.
