...
from sample_factory.utils.utils import log, AttrDict, cfg_file, get_git_commit_hash, str2bool
...
def arg_parser(argv=None, evaluation=False):
    ...
    parser.add_argument('--env', type=str, default='nle_fixed_eat_action', required=False, help='Fully-qualified environment name in the form envfamily_envname, e.g. atari_breakout or doom_battle')

    parser.add_argument('--msg_count_bonus', default=0.0, type=float, help='The coefficient for the counts bonus on messages only.')
    parser.add_argument('--beta_count_exponent', default=3, type=float, help='The exponent for the counts.')
    parser.add_argument('--eps_threshold_quantile', default=-0.5, type=float, help='The threshold over which the reward is activated')
    parser.add_argument('--rew_norm', default=True, type=str2bool, help='To normalize or not the intrinsic reward.')
    parser.add_argument('--root_env', type=str, default='NetHackScore-v1', required=False, help='Fully-qualified environment name in the form envfamily_envname, e.g. atari_breakout or doom_battle')
